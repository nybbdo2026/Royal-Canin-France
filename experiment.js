      ...
      stimulus: () => {
        return `
          <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:70vh;">
            <p style="font-size:3rem; text-align:center; font-weight:bold;">Combien font 3 × 2 ?</p>
            ${
              respondentIsMobile 
                ? ''
                : `
...
const attentionCheck4 = {
  ...
  stimulus: () => {
    return `
      <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:70vh;">
        <p style="font-size:3rem; text-align:center; font-weight:bold; margin-bottom:4vh;">Combien font 4 × 6 ?</p>
...
function generateFlatTrials(trialVars, respondentId, partLabel) {
  ...
  trialVars.forEach(vars => {
    const trial = {
      ...
      `,
      choices: respondentIsMobile ? ['Correspond à', 'Ne correspond pas à'] : ['e', 'i'],
      button_html: respondentIsMobile
        ? (choice, index) => `
          <button style="
      ...
      on_finish: function(data) {
        let label;
        if (respondentIsMobile) {
          label = data.response === 0 ? "Correspond à" : "Ne correspond pas à";
        } else {
          label = data.response === 'e' ? "Correspond à" : "Ne correspond pas à";
        }
        data.selected_label = label;

        if (vars.is_correct !== undefined) {
          data.accurate = (label === (vars.is_correct ? "Correspond à" : "Ne correspond pas à"));
        }
  ...
  trialVars.forEach(vars => {
    ...
    const trial = {
      ...
      stimulus: () => {
        ...
        const brandImgs = imagePaths;
        const promptText = isPretest ? "Quelle image représente le mieux :" : "Quelle marque représente le mieux :";
        const brandKeys = ['A', 'S', 'K', 'L'];
        const brandKeyColors = [
          "rgb(32, 150, 11)",
          "rgb(60, 145, 237)",
...
`  ,  choices: respondentIsMobile ? ['Correspond à', 'Ne correspond pas à'] : ['e', 'i'],
    ...
    on_finish: function(data) {
      ...
      data.user_answer = userSaysFits ? "Correspond à" : "Ne correspond pas à";
      data.correct_answer = data.is_correct ? "Correspond à" : "Ne correspond pas à";
      data.accurate = (userSaysFits === data.is_correct);
    }
  };
  if (respondentIsMobile) {
    ...
    timeline: [
...

      {
        ...
        stimulus: function () {
          ...
          if (lastReal.rt < minRT) {
            return `<p style="font-size:2rem; color:red;">
                      ⚡ Trop rapide ! Veuillez ralentir.<br>
                      ${respondentIsMobile ? "" : "Appuyez sur n’importe quelle touche pour recommencer."}
                    </p>`;
          }
          if (lastReal.rt > maxRT) {
            return `<p style="font-size:2rem; color:red;">
                      🐢 Trop lent ! Veuillez répondre plus rapidement.<br>
                      ${respondentIsMobile ? "" : "Appuyez sur n’importe quelle touche pour recommencer."}
                    </p>`;
          }
          return "";
  ...
  stimulus: `
    <div style="
      ...
      <p1 style="font-size: clamp(1.6rem, 4.0vw, 2rem); font-weight: 600; margin-bottom: 2vh;">
        Bienvenue dans notre enquête sur les associations implicites !
      </p>
...
timeline.push({
  type: respondentIsMobile ? jsPsychHtmlButtonResponse : jsPsychHtmlKeyboardResponse,
  stimulus: `<div>
  <h2 style = "font-size: 2.0rem" > Nous allons commencer par un prétest afin d’établir une référence de base. </h2>
  </div>
  ${
    respondentIsMobile
    ? ""
    : "<p style='font-size: 18px> ;'>Appuyez sur n’importe quelle touche pour commencer. </p>"
  }`,
  save_trial_parameters: {
    simulus: false
  },
...
timeline.push({
  ...
  stimulus: `
    <div style="text-align:center; font-size: 2rem; font-weight: 500;">
      <p>Merci.</p>
      <p>Le véritable test commencera après ceci.</p>
      ${
        respondentIsMobile
...
const single_implicit_completition_trial = {
  ...
  stimulus: `
    <div style="text-align:center;">
      <p style="font-size: 3rem;">Partie 1 terminée !</p>
      ${
        respondentIsMobile
          ? '' // mobile will have a button
          : '<p>Appuyez sur n’importe quelle touche pour passer à la partie 2.</p>'
      }
    </div>
  `,
  button_html: respondentIsMobile
...
const multiImplicitIntroTrial = {
  ...
  stimulus: `
    <div style="text-align: center; font-size: 2rem; font-weight: 500;">
      <p>Ensuite, nous commencerons par un prétest afin d’établir une référence de base pour notre partie d’associations implicites multiples.</p>
      <p></p>
      ...
      ${
        respondentIsMobile
          ? '' // Mobile uses button
          :  `
  <p>Veuillez placer vos deux mains sur le clavier, avec vos index et majeurs sur les touches A, S, K et L.</p> 
...
const multiple_pretest_completion_trial = {
  ...
  stimulus: `
    <div style="text-align: center; font-size: 2rem; font-weight: 500;">
      <p>Prétest multiple terminé !</p>
      <p>La tâche principale commencera ensuite.</p>
      ${
        respondentIsMobile
          ? '' // Mobile uses styled button
          : '<p style="font-size: 1.5rem; color: #666;">Appuyez sur n’importe quelle touche pour passer à la tâche principale.</p>'
      }
    </div>
  `,
  button_html: respondentIsMobile
...
timeline.push({
  ...
  stimulus: `
    <div style="text-align: center; font-size: 2rem;">
      ...
      <p>Veuillez garder cette fenêtre ouverte jusqu’à la redirection.</p>
    </div>
  `,
  choices: "NO_KEYS",
  trial_duration: 1000,
